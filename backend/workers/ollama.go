...
func StartOllamaWorker() {
	...
	go func() {
		...
		log.Printf("Ollama worker started, polling every %d seconds", pollIntervalSec)
...
func pollOllama() {
	...
	if err != nil {
		...
		log.Printf("Warning: Failed to poll Ollama at %s: %v", url, err)
	...
	if resp.StatusCode != http.StatusOK {
		log.Printf("Warning: Ollama returned status %d", resp.StatusCode)
		setOllamaModelsOffline()
		return
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&tags); err != nil {
		log.Printf("Warning: Failed to decode Ollama response: %v", err)
		return
	}
	...
	for _, om := range tags.Models {
		...
		if err == sql.ErrNoRows {
			...
			log.Printf("Database query error in Ollama worker: %v", err)
			continue
		} else {
			exists = true
		}
		...
		if err != nil {
			log.Printf("Failed to upsert Ollama model %s: %v", om.Name, err)
		}
	}
	...
	if err != nil {
		log.Printf("Failed to query models for offline check: %v", err)
		return
	}
	defer rows.Close()
	...
	for _, id := range modelsToUpdate {
		...
		if err != nil {
			log.Printf("Failed to update status for offline model %s: %v", id, err)
		}
	}
...
func setOllamaModelsOffline() {
	...
	`, now)
	if err != nil {
		log.Printf("Failed to set Ollama models offline: %v", err)
	}
	ws.Manager.Broadcast("models", map[string]string{"type": "update", "time": now.Format(time.RFC3339)})
}
