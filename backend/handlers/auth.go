...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := ParseJSON(r, &req); err != nil {
		log.Printf("Login: Failed to parse JSON request body: %v", err)
		WriteError(w, http.StatusBadRequest, "Invalid request body")
		return
	}

	if req.Email == "" || req.Password == "" {
		log.Printf("Login: Missing email or password in request")
	...
	if err != nil {
		log.Printf("Login failed: email %q not found in DB or error: %v", req.Email, err)
		WriteError(w, http.StatusUnauthorized, "Invalid email or password")
		return
	}

	if !user.IsActive {
		log.Printf("Login failed: account %q is disabled", req.Email)
		WriteError(w, http.StatusForbidden, "Account is disabled")
		return
	}

	if !auth.CheckPasswordHash(req.Password, user.HashedPassword) {
		log.Printf("Login failed: password check failed for %q (hashed_pwd: %s)", req.Email, user.HashedPassword)
		WriteError(w, http.StatusUnauthorized, "Invalid email or password")
		return
	}

	log.Printf("Login success: email=%q role=%q ID=%s", req.Email, user.Role, user.ID)
