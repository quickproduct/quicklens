...
func StartIngestWorker() {
	go func() {
		zap.L().Sugar().Info("Ingest worker started")
...
func persistTrace(req models.TraceIngestRequest) {
	...
	if err != nil {
		zap.L().Sugar().Infof("Failed to insert trace: %v", err)
		return
	}
	...
	for _, s := range req.Spans {
		...
		if err != nil {
			zap.L().Sugar().Infof("Failed to insert span: %v", err)
		}
	}
	...
	zap.L().Sugar().Infof("Ingested trace %s with %d spans", traceID, len(req.Spans))
}
...
func IngestOTelHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, rs := range otlpPayload.ResourceSpans {
		for _, ss := range rs.ScopeSpans {
			...
			default:
				zap.L().Sugar().Info("Ingest queue full, dropping OTLP trace")
			}
		}
	}
