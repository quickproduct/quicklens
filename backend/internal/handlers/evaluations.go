...
func CreateEvaluationHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		zap.L().Sugar().Infof("Failed to insert evaluation: %v", err)
...
func ListEvaluationsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		zap.L().Sugar().Infof("Failed to query evaluations: %v", err)
	...
	for rows.Next() {
		...
		if err != nil {
			zap.L().Sugar().Infof("Failed to scan evaluation: %v", err)
			continue
		}
...
func GetSpanEvaluationsHandler(w http.ResponseWriter, r *http.Request) {
	...
	`, spanID)
	if err != nil {
		zap.L().Sugar().Infof("Failed to query span evaluations: %v", err)
		WriteError(w, http.StatusInternalServerError, "Failed to query evaluations")
		return
	}
	defer rows.Close()
